    ...
    <div class="card-inner">
      <div class="card-title-group align-start mb-2">
        <div class="card-title">
            <h6 class="title eg-swal-default">List Data Trasit Perwakilan Packet - {{ auth()->user()->merchant_id }}</h6>
  ...
  function getTablesData() {
    ...
    async function loadall() {
        try{
            ...
            console.log(dtgetDataMasterHistoryDelivery);
        } catch (err) {
           // toast(err, "error")
           console.log(err);
        }
    ...
    function goValidate() {
       ...
       var id_packet =  $('#id_packet').val();
        if(No_resi.length == "17"){
          updateDataTransit(id_packet,No_resi);
        }
    }
    async function updateDataTransit(id_packet,No_resi) {
        try{
            const dtGoTransitDataPacket= await GoTransitDataPacket(id_packet,No_resi); 
    ...
    function GoTransitDataPacket(id_packet,No_resi) {
       var base_url = window.location.origin;
        let url = base_url + '/dashboard/transit/create'; 
        ...
        return fetch(url, {
            ...
            body : "id_packet=" + id_packet + "&No_resi=" + No_resi 
        })
            .then(response => {
                if (!response.ok) { throw new Error(response.statusText) }  return response.json();
            })
